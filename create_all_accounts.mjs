...
// Read from an environment variable instead of hardcoding it in this file.
// This file gets uploaded to a PUBLIC GitHub repo -- a key typed directly
// into the source would be visible to anyone on the internet. Setting it in
// the terminal (see the export command) keeps it out of every file, so
// there's nothing to accidentally commit or mistype inside an editor.
const SUPABASE_SECRET_KEY = process.env.SUPABASE_SECRET_KEY || "";

if (!SUPABASE_SECRET_KEY) {
  console.error("Missing key. In the terminal, run this FIRST (with your real key):");
  console.error('  export SUPABASE_SECRET_KEY="sb_secret_..."');
  console.error("Then run: node create_all_accounts.mjs");
  process.exit(1);
}
